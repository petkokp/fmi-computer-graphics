...
def fill_circle_symmetric_difference(ax, center1, radius1, center2, radius2, fill_color):
    ...
    fill_recursive(ax, symmetric_difference_polygon,
                   fill_color='green', alpha=0.5)
    ...
    try:
        ...
        if point.within(symmetric_difference_polygon):
            print(
                f"The point ({point_x}, {point_y}) is within the symmetric difference area.")
            ...
            fill_recursive(ax, symmetric_difference_polygon,
                           fill_color='red', alpha=1)

            print("Filling completed.")
            plt.show()
        else:
            print(
                f"The point ({point_x}, {point_y}) is not within the symmetric difference area.")
    except ValueError:
...
def fill_circle_difference(ax, center1, radius1, center2, radius2, fill_color):
    ...
    try:
        ...
        if point.within(circle1) and not point.within(circle2):
            ...
            print(
                f"Точката ({point_x}, {point_y}) не е в областта на разликата.")
    except ValueError:
        print("Невалиден вход.")
    except KeyboardInterrupt:
        print("\nПроцесът е прекъснат.")
...
def onclick(event):
    ...
    if not center1:
        ...
        fill_circle_difference(ax, center1, radius1,
                               center2, radius2, fill_color='green')
